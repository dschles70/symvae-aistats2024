...
class FNET1x(torch.nn.Module):
    ...
    def forward(self,
                z1 : torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = scores_x.sigmoid().bernoulli().detach()
...
class HVAE(torch.nn.Module):
    def __init__(self, 
        ...
        # something in addition to watch mutual informations
    ...
    # single-shot (i.e. sampling from random code)
    def single_shot(self,
        ...
        with torch.no_grad():
            _, z0 = self.fnet0(bs)
            _, z1 = self.fnet01(z0)
            x_scores, _ = self.fnet1x(z1)
            return x_scores.sigmoid()

    # for sampling from limiting distribution, i.e. x->z0/z1->x
    def limiting(self,
    ...
    # update mutual informations for p
    ...
    # update mutual informations for q
